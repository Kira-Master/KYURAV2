...
└ ${prefix}ytmp4 [link yt] = Video`})
   ...
   if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(mess.limit) 
				...
				case 'sticker':
					...
					case 's':
						if (
          ((isMedia && !kira.message.videoMessage) || isQuotedImage) &&
          args.length == 0
        ) {
          const encmedia = isQuotedImage
            ? JSON.parse(JSON.stringify(kira).replace("quotedM", "m")).message
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              kira.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: kira,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
          ...
          ((isMedia && kira.message.videoMessage.seconds < 11) ||
            (isQuotedVideo &&
              kira.message.extendedTextMessage.contextInfo.quotedMessage
