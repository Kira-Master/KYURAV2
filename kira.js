...
try {
        ...
        mess = {
		    ...
		    regist: '```Hai kak😄, sepertinya anda belum terdaftar```\n```Ketik``` *verify* ```untuk mendaftar```',
			success: 'Success',
			error: {
				stick: 'Cannot access videos!',
				Iv: 'Invalid link!',
                api: 'something error,please wait!'
			},
			only: {
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					ppimg = await kira.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
...
❀ *ᴀꜱᴀʟ ᴋᴏᴛᴀ* :
 ...
 ❲ *SEMOGA BETAH* ❳`
				...
				kira.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
if (lmt.id === sender) {
limitCounts = settings.limitCount - lmt.limit
if (limitCounts <= 0) return kira.sendMessage(from, `Maaf limit kamu telah habis, silahkahkan ketik *owner* untuk meminta limit`, text,{ quotedM: m})
kira.sendMessage(from, `〘  *Limit* 〙
...
kira.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© KIRA MASTER*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			kira.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6282322153431?text=Assalamualaikum`}}})
		}
		const simir = (teks) => {
...
return reply(parse)
           ...
           if (budo.startsWith(`>`)){
           	if (!isOwner) return
                ...
                const print = function(budo){
                    kira.sendMessage(from, util.format(budo))
                }
                console.log(exc)
                eval("(async () => {try{"+exc+"}catch(e){kira.sendMessage(from,  e.toString())}})()")
			}
			if (budo.startsWith(`$`)){
				if (!isOwner) return
				const sep = budo.split("\n")
                ...
                exec(exc, (err, stdout) => {
					...
					if (stdout) {
						kira.sendMessage(from, stdout, text)
					}
				})
			}
			// AUTO
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
			    ...
			    kira.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
...
await kira.modifyChat(m.chat, 'delete', {
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© KIRA MASTER_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`command`,buttonText:{displayText:'SIMPLE MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
			case 'allmenu':
kira.sendMessage(from, ' *wait,I will reveal the menu* ', text, {quoted: ftok})
stst = await kira.getStatus(`${sender.split('@')[0]}@c.us`)
stst = stst.status == 401 ? '' : stst.status
num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
...
case 'command':
 ...
 footerText: '*_KIRA-MASTER_* 右',
 description: `Hai kak, Silahkan pilih menu disini,kalau anda memakai Whatsapp MOD silahkan ketik *allmenu*`,
 sections: [
                     {
...
case 'donasi':
case 'donate':		
kira.sendMessage(from, donasi(NamaBot, gopay, pulsa, dana, shopeepay, ovo), text)
		break
case 'giftlimit':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (!c)return reply(`Example : ${prefix + command} @tag 10`)
lim = c.split(" ")[1]
kentodd = `*_CLICK SALAH SATU UNTUK MELAKUKAN TRANSAKSI_*`
sendButMessage(from, kentodd, "K Y U R A 右", [
            {              
              buttonId: `${prefix}gift ${c} ${lim}`,
              buttonText: {
                displayText: `🔖LIMIT`,
              },
              type: 1, 
...
if (!c)return reply(`Example : ${prefix + command} @tag 10`)
lim = c.split(" ")[1]
const tag1 = `${c.split(" ")[0].replace("@",'')}@s.whatsapp.net`
giveLimit(tag1, lim, _limit) 
...
if (isLimit(sender)) return reply(mess.limit(pushname))
...
client.browserDescription = ['KiraMaster','Firefox','3.0']
...
case 'premiumlist':
				...
				kira.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": premium}})
				break
case 'stopjadibot':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
try {
...
• *WAKTU VERIFIKASI :* ${time}
      ...
      「 *TERIMAKASIH* 」`
kira.sendMessage(from, hasil, text, {quotedM: m})
...
break
	...
	if (!isRegister) return reply(mess.regist)
if (isLimit(sender)) return reply(mess.limit(pushname))
		kira.sendMessage(from, { text: "https://github.com/Kira-Master/KYURAV2", matchedText: 'https://github.com/Kira-Master', description: "", title: "Kira-Master", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		await limitAdd(sender)
		break
       case 'debug':
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/fauzi1232"
        },
        "index": 0
      }
    ]
...
kira.sendMessage(from, kon, image, { quoted: mek, thumbnail: fs.readFileSync('./kira.jpg')})
await limitAdd(sender)
break
case 'lesbian':
if (isBanned) return reply(mess.ban)
...
kira.sendMessage(from, hai, image, { quoted: mek, thumbnail: fs.readFileSync('./kira.jpg')})
await limitAdd(sender)
break
case 'wallpaper':
if (isBanned) return reply(mess.ban)
...
kira.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./kira.jpg')})
await limitAdd(sender)
break
case  'blowjob':
if (isBanned) return reply(mess.ban)
...
kira.sendMessage(from, img, image, { quoted: mek, thumbnail: fs.readFileSync('./kira.jpg')})
await limitAdd(sender)
break
case 'loli':
if (isBanned) return reply(mess.ban)
...
case 'xsearch':
case 'xs':
if (!isPremium) return reply("*Beli Premium Dulu Kontol,Ngocok Juga Perlu Modal!!!!*")
if (isBanned) return reply(mess.ban)
	if (!isRegister) return reply(mess.regist)
if (isLimit(sender)) return reply(mess.limit(pushname))
...
kira.sendMessage(from, anu, text, {quoted: mek})
await limitAdd(sender)
break
case 'xvideo':
case 'xv':
...
kira.sendMessage(from, vid, video, {quoted: mek})
await limitAdd(sender)
break
case 'matrix':
if (isBanned) return reply(mess.ban)
...
break
case 'tts':
					if (args.length < 1) return kira.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return kira.sendMessage(from, `Teksnya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
					...
					bby.length > 300
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								kira.sendMessage(from, buff, audio, { duration: 359996400, ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
...
break
             ...
             fak = kira.query({ json: ['action', 'invite', link],
             expect200: true })
...
case 'buglink':
                if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                 kira.toggleDisappearingMessages(from, 0)
               kira.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '🔥',
            ...
            "externalAdReply": {
                ...
                "body": "",
                "previewType": "PHOTO",
                "thumbnailUrl": "https://i.ibb.co/3hrZZ6s/kira.png",
...
kira.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
kira.toggleDisappearingMessages(from, 0)
break
					case 'bugpc2':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
└ ❏ *Link:* ${items[i].url}\n\n`
                ...
                await kira.sendMessage(from, thumb, image, {quoted: mek, caption: `${hehe}───────────────\n\n┌ ◪ *DOWNLOAD*
...
case 'nsfw':
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					if (Number(args[0]) === 1) {
						...
						kira.sendMessage(from, `Bebas Nyari Hentai :v`, text)
					} else if (Number(args[0]) === 0) {
...
Giliran = @${tty.player1.split('@')[0]}`
 kira.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kira.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Kyura Boot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./kira.jpg'),sourceUrl:"https://wa.me/6282322153431?text=Assalamualaikum"}}})
	}
    }
    }
    }
